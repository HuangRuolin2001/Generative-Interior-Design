...
# 使用 CPU 设备
device = "cpu"
dtype = torch.float32  # CPU 不支持 float16
...
class ControlNetDepthDesignModelMulti:
    ...
    @spaces.GPU
    def generate_design(self, empty_room_image: Image, prompt: str, guidance_scale: int = 10, num_steps: int = 50,
                        strength: float = 0.9, img_size: int = 640) -> Image:
        """
        ...
        # 定义 GLIGEN 的布局参数
        gligen_boxes = [
            [0.25, 0.25, 0.75, 0.75],  # 示例布局，表示一个对象的位置
        ]
        gligen_phrases = ["a grand king-size bed"]  # 与 gligen_boxes 对应的文本描述

        ip_image = gligen_pipe(
            prompt=pos_prompt,
...
def create_demo(model):
    ...
    def on_submit(image, text, num_steps, guidance_scale, seed, strength, a_prompt, n_prompt, img_size):
        ...
        with torch.no_grad():
            out_img = model.generate_design(image, text, guidance_scale=guidance_scale, num_steps=num_steps,
                                            strength=strength, img_size=img_size)

        return out_img

    submit.click(on_submit,
                 inputs=[input_image, input_text, num_steps, guidance_scale, seed, strength, a_prompt, n_prompt,
                         img_size], outputs=design_image)
    examples = gr.Examples(examples=[["imgs/bedroom_1.jpg",
                                     ...
                                     ["imgs/bedroom_2.jpg",
                                      "A bedroom that exudes French country charm with a soft upholstered bed, walls adorned with floral wallpaper, and a vintage wooden wardrobe. A crystal chandelier casts a warm, inviting glow over the space"],
                                     ["imgs/dinning_room_1.jpg",
                                     ...
                                     ["imgs/dinning_room_3.jpg",
                                      "A dining room that epitomizes contemporary elegance, anchored by a sleek, minimalist dining table paired with stylish modern chairs. Artistic lighting fixtures create a focal point above, while the surrounding minimalist decor ensures the space feels open, airy, and utterly modern"],
                                     ["imgs/image_1.jpg",
...
def main():
    ...
    title = "# StableDesign"
    description = """
    <p style='font-size: 14px; margin-bottom: 10px;'>Du lei, Huang Ruolin, Sun che, Liu Zhuoxin</p>
    <p style='font-size: 14px; margin-bottom: 10px;'>Advisor: Ovanes Petrosyan</p>
    """
